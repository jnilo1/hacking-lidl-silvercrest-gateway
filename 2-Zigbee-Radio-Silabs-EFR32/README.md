...
> **Per-board builds:** every firmware build takes a `BOARD=` parameter
> (default `lidl`). For the **Sengled Smart Hub G4** use
> `BOARD=sengled-e39-g8c` — its NCP is validated on real G4 hardware (#130) and
> ships **prebuilt**, so a G4 user can flash it directly
> (`BOARD=sengled-e39-g8c ./flash_efr32.sh -y ncp`); G4 OT-RCP, Router,
> bootloader and RCP (#143) build but aren't hardware-validated yet, so build
> them yourself. See [`boards/README.md`](./boards/README.md) for the supported
> boards and the porting contract.
