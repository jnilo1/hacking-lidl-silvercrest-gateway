...
# build_router.sh, build_bootloader.sh). The board's UART facts come from
# boards/<board>/board.env (BOARD_UART_*); these helpers write them into the
# slc-generated UART config header.
#
# They substitute ONLY the value token on each `#define` line, preserving the
...
#
# Usage: apply_uart_routing <header> <define-prefix>
#   Routing-only subset (USART instance + TX/RX data lines) for headers that
#   carry no CTS/RTS or flow-control-type defines — the Gecko bootloader's
#   btl_uart_driver_cfg.h (prefix SL_SERIAL_UART), whose flow control is a
#   separate numeric knob deliberately left at 0 for every board (the Xmodem
...
apply_uart_routing() {
    ...
    # Each BOARD_UART_{TX,RX} is "<port-letter> <pin> <loc>".
...
apply_uart_config() {
    ...
    # Each BOARD_UART_{CTS,RTS} is "<port-letter> <pin> <loc>".
