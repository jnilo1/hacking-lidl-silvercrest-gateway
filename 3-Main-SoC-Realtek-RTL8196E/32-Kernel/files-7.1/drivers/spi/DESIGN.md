...
1. Pick the divisor for this transfer's `speed_hz` (first ÷ giving
   rate ≤ request; falls back to ÷16 — see AUDIT SPI-004) and rewrite
   CONFIG.
...
multi-beat transfer from glitching CS. Known wart: the deselect branches
park only the *addressed* line high (AUDIT SPI-001); correct today because
CS1 has no listener.
