...
#     and either restores the original trigger or leaves it parked at "none"
#     when LEAVE_DIM=1.
#
#   * LAN LED — switch-ASIC LED_PORTn output (pad B6 on Lidl, B2 on
#     Sengled G4). The rtl8196e-eth driver exposes it as a sysfs knob:
#       ON  = bright  (LEDCREG=DIRECT, DIRECTLCR=default — full link/act)
#       DIM = dim     (LEDCREG=0 scan mode, ≈25% brightness)
#       OFF = off     (pad remuxed to GPIO, active-low output held high)
#     Path: /sys/class/net/eth0/led_mode
#     It is a link/activity LED: with the cable up it glows and flickers
#     on traffic; "off" disconnects the ASIC regardless of link. Toggling
#     it does NOT touch the PHY, so the SSH link stays up throughout.
#
# By default the test leaves both LEDs in DIM so that the operator can reboot
# immediately and check whether the board/driver preserves that state. Set
# LEAVE_DIM=0 to restore the entry state instead (including Ctrl-C).
#
...
PIN_MUX_SEL2_PHYS=0x18000044
PABCD_CNR_PHYS=0x18003500
PABCD_DIR_PHYS=0x18003508
PABCD_DAT_PHYS=0x1800350C
...
MODEL=$(echo "$PREFLIGHT" | sed -n 2p)
info "Kernel: ${KREL}"
info "Board: ${MODEL}"
case "$MODEL" in
    *Sengled*)
        LAN_PAD="B2/LED_PORT0"
        LAN_MUX_MASK="0x00000003"
        LAN_GPIO_BIT="0x00000400"
        ;;
    *)
        LAN_PAD="B6/LED_PORT4"
        LAN_MUX_MASK="0x00003000"
        LAN_GPIO_BIT="0x00004000"
        ;;
esac
HAVE_DEVMEM=0
...
restore(){
    ...
    if [ "$LEAVE_DIM" = "1" ]; then
        log "Leaving both LEDs DIM (status=${STATUS_DIM}; lan=dim)"
        gw "
            echo none > ${STATUS_LED}/trigger 2>/dev/null
            echo ${STATUS_DIM} > ${STATUS_LED}/brightness 2>/dev/null
            echo dim > ${LAN_LED} 2>/dev/null
            true
        " 2>/dev/null
    else
...
read_lan(){
    ...
    if [ "$HAVE_DEVMEM" = 1 ]; then
        local regs
        regs=$(gw "echo LEDCREG=\$(devmem ${LEDCREG_PHYS} 32) DIRECTLCR=\$(devmem ${DIRECTLCR_PHYS} 32) PINMUX=\$(devmem ${PIN_MUX_SEL2_PHYS} 32) CNR=\$(devmem ${PABCD_CNR_PHYS} 32) DIR=\$(devmem ${PABCD_DIR_PHYS} 32) DAT=\$(devmem ${PABCD_DAT_PHYS} 32)" 2>/dev/null)
        echo "${mode}   [${regs}]"
    else
        echo "${mode}"
    fi
...
echo
log "═══ LAN LED (${LAN_PAD}, ASIC for ON/DIM, GPIO for OFF) ═══"
info "Expected board masks: mux=${LAN_MUX_MASK}, gpio=${LAN_GPIO_BIT}"
...
confirm "LAN LED is fully OFF (dark, even with the cable plugged in; pad routed to inactive GPIO)"
