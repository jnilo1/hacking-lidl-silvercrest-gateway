...
# hardware reset which leaves a v4 post-mortem record in the reserved DRAM
...
#     - driver reports "record v4", and the last-reset cause line
...
#     Fires `echo c > /proc/sysrq-trigger` (NULL-deref → oops →
#     panic_on_oops → panic).  The wdt panic notifier writes the v4 record
#     and arms a ~1.31 s reset.  The script then waits for the reboot and
#     checks:
#       - the box came back
#       - dmesg carries "previous boot ended in panic: ... reason=..."
#       - the record is v4 (softirqs=[..] hardirqs=.. napi=[..] fields)
...
#
# A sysrq panic is process-context (no IRQ regs), so pc/ra/timers/napi are
# expected EMPTY — the point here is the record write→survive-reset→
...
gw 'dmesg 2>/dev/null | grep -i "rtl819x-wdt" | grep -q "record v4"' 2>/dev/null \
    && ok "driver reports record v4" || warn "driver did not report record v4 (older driver?)"
...
EXIST=$(gw "dmesg 2>/dev/null | grep -F 'previous boot ended in panic'" 2>/dev/null)
...
if [ "${SYSRQ:-0}" = "0" ] || [ "${PANICOOPS:-0}" = "0" ]; then
    die "sysrq (=$SYSRQ) or panic_on_oops (=$PANICOOPS) disabled — sysrq-c would not panic. Aborting."
fi

echo
log "═══ Phase 2 — panic → watchdog reset → v4 record ═══"
...
sleep 2
REC=$(gw "dmesg 2>/dev/null | grep -F 'previous boot ended in panic'" 2>/dev/null)
if [ -n "$REC" ]; then
    ok "panic record decoded on reboot"
    echo "$REC" | sed 's/^/      /'
else
    bad "no 'previous boot ended in panic' line in dmesg — record not written/read"
fi

# v4 fields present?
if echo "$REC" | grep -q "softirqs=" && echo "$REC" | grep -q "hardirqs=" && echo "$REC" | grep -q "napi="; then
    ok "record is v4 (softirqs/hardirqs/napi fields present)"
else
    bad "v4 fields (softirqs=/hardirqs=/napi=) missing — record older than v4?"
fi
...
HIST=$(gw "cat ${PANIC_HIST} 2>/dev/null" 2>/dev/null)
if echo "$HIST" | grep -qF 'previous boot ended in panic'; then
