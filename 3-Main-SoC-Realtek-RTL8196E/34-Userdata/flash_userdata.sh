...
if [ ! -f "$IMAGE" ]; then
    echo "Error: userdata.bin not found"
    echo "Run ./build_userdata.sh first"
    exit 1
fi
...
if [[ ! "$confirm" =~ ^[yY]$ ]]; then
    echo "Aborted."
    echo "To flash manually: tftp -m binary ${TARGET_IP} -c put userdata.bin"
    exit 0
fi

echo "Uploading..."
cd "$SCRIPT_DIR"
out=$(tftp -m binary -timeout 10 "$TARGET_IP" -c put userdata.bin 2>&1) || true
