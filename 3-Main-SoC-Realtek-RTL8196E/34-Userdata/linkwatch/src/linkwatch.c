 ...
 *
 * Written in C, like keepalive / s40button: long-lived busybox-ash poll loops
 * were then taking intermittent SIGSEGV/SIGILL here (issue #109). That fault
 * class has since been root-caused to our own TLB flush — it swept from a
 * hardcoded index instead of the Wired boundary, leaving slots 0-7 never
 * invalidated — and fixed, so it is no longer a reason to avoid ash. C stays
 * for the ordinary reason: a daemon blocked in nanosleep() forks nothing.
 *
