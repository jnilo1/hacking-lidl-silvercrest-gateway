 ...
 * only process still exposed to the intermittent SIGSEGV/SIGILL/SIGBUS faults
 * that were hitting long-lived ash loops at the time (issue #109 — the same
 * class that retired the s40button and inline-monitor shell loops).
 *
 * That fault was NOT an RLX4181/Lexra silicon defect, as this comment used to
 * claim: it was root-caused to our own local_flush_tlb_all(), which swept from
 * a hardcoded TLB index inherited from classic R3000 instead of the Wired
 * boundary — 0 on this core, so slots 0-7 were never invalidated. Fixed. ash
 * is not inherently unsafe here, and new code should not cite this fault as a
 * reason to avoid it.
 *
 * The C rewrite is kept on its own merits, which are the ones below: no ash is
 * spawned here, so the housekeeping loop costs no fork/exec per cycle. The REST
 * API is read over a plain TCP
